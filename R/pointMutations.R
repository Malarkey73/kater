...
pointMutations= function(DF)
{
  
  #C>A is the same as G>T on the -strand 
  CtoA<- DF %>% dplyr::summarise(CA=sum((ref=="C" & mut=="A")|(ref=="G" & mut=="T")))
  #C>G is the same as G>C on the -strand 
  CtoG<- DF %>% dplyr::summarise(CG=sum((ref=="C" & mut=="G")|(ref=="G" & mut=="C")))
  #C>T is the same as G>A on the -strand 
  CtoT<- DF %>% dplyr::summarise(CT=sum((ref=="C" & mut=="T")|(ref=="G" & mut=="A")))
  
  #T>A is the same as A>T on the -strand 
  TtoA<- DF %>% dplyr::summarise(TA=sum((ref=="T" & mut=="A")|(ref=="A" & mut=="T")))
  #T>C is the same as A>G on the -strand 
  TtoC<- DF %>% dplyr::summarise(TC=sum((ref=="T" & mut=="C")|(ref=="A" & mut=="G")))
  #T>G is the same as A>C on the -strand 
  TtoG<- DF %>% dplyr::summarise(TG=sum((ref=="T" & mut=="G")|(ref=="A" & mut=="C")))
